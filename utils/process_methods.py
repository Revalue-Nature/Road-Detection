...
def channel_check(x, n_channel):
    ...
    if _x.shape[-1] == 4:
        new_data = _x[:, :, :3]
        # new_data = np.dstack((_x, new_channel[..., None]))
        xx = new_data
    else:
        xx = x
        new_data = x
    if new_data.shape[-1] != n_channel:
        assert n_channel == 1 and new_data.shape[-1] != 3, 'Expect channel {}, actual shape {}.'.format(n_channel, x.shape)
        xx = np.reshape(new_data, list(new_data.shape) + [1])
    return xx
